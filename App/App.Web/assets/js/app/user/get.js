    ...
    $.ajax({
        ...
        success: function (msg1) {
            ...
            if (msg.result) {
                ...
                var base64_akta_pendirian = msg.objek.base64_akta_pendirian;
                var base64_surat_pengesahan = msg.objek.base64_surat_pengesahan;
                var base64_surat_permohonan = msg.objek.base64_surat_permohonan;
                ...
                if (base64_akta_pendirian !== null) {
                    html += `<tr>
                                            <th>Akta Pendirian</th>
                                            <td>
                                                <input type="hidden" value="${base64_akta_pendirian}" id="value_akta_pendirian">
                                                <a href="javascript:;" onclick="UnduhFilePerusahaan('value_akta_pendirian')">Unduh</a>
                                            </td>
                                        </tr>`;
                }

                if (base64_surat_pengesahan !== null) {
                    html += `<tr>
                                            <th>Surat Pengesahan</th>
                                            <td>
                                                <input type="hidden" value="${base64_surat_pengesahan}" id="value_surat_pengesahan">
                                                <a href="javascript:;" onclick="UnduhFilePerusahaan('value_surat_pengesahan')">Unduh</a>
                                            </td>
                                        </tr>`;
                }

                if (base64_surat_permohonan !== null) {
                    html += `<tr>
                                            <th>Surat Permohonan</th>
                                            <td>
                                                <input type="hidden" value="${base64_surat_permohonan}" id="value_surat_permohonan">
                                                <a href="javascript:;" onclick="UnduhFilePerusahaan('value_surat_permohonan')">Unduh</a>
                                            </td>
                                        </tr>`;
                }
...
function ModalPreviewDetail(id) {
    OpenLoading();
    $.ajax({
        type: "GET",
        url: "/User/GetDataDetail?id_user_info=" + id,
        success: function (msg1) {
            CloseLoading();
            var msg = JSON.parse(msg1);
            if (msg.result) {
                var user_info = msg.objek.user_info;
                var list_user_package = msg.objek.list_user_package;
                ...
                for (var lu in list_user_package) {
                    if (parseInt(lu, 10) === 0) {
                        html += `<tr>
                                            <th>Package</th>
                                            <td>
                                                 ${list_user_package[lu].package_string}, Expired Date : ${UbahWaktu(list_user_package[lu].expired_date)}
                                            </td>
                                        </tr>`;
                    }
                    else {
                ...
                $('#model_preview_detail').modal('show');
            }
            else {
                toastr.error(msg.message);
            }
...
function ModalPreviewCertificate(user_name, id_user, name) {
    OpenLoading();
    $('#id_user_renewal').val(id_user);
    $('#name_renewal').val(name);
    $.ajax({
        type: "GET",
...
function ModalRenewal(user_name, serial_number) {
    $('#span_user_name_renewal').html(user_name);
    $('#user_name_renewal').val(user_name);
    $('#serial_number_renewal').val(serial_number);
    $('#modal_renewal').modal('show');
}

function RenewalCertificate() {
    $('#modal_renewal').modal('hide');
    $('#model_preview_certificate').modal('hide');
    OpenLoading();
    var post_renewal = {
        serial_number: $('#serial_number_renewal').val(),
        username: $('#user_name_renewal').val(),
        subject_dn: $('#subject_dn_renewal').val(),
        id: $('#id_user_renewal').val(),
        name: $('#name_renewal').val()
    };
    var anti_forgery_token = $('meta[name="anti_forgery_token"]').attr('content');
    $.ajax({
        type: "POST",
        url: "/Certificate/Renewal",
        data: post_renewal,
        headers: { '__RequestVerificationToken': anti_forgery_token, '_ip_user': $('#_ip_user').val() },
        success: function (msg) {
            CloseLoading();
            if (msg.result) {
                SetMessage('1', 'Success rekeying certificate');
                window.location.href = '/User/Get';
            }
            else {
                toastr.error('error');
...
function RaVerification() {
    var id_ekyc_ra = $('#id_ekyc_ra').val();
    window.open('/Ekyc/RaVerification?id_ekyc=' + id_ekyc_ra, '_blank');
}
...
function DeleteUser() {
    ...
    $.ajax({
        type: "POST",
        url: "/User/Delete?id_user_info=" + $('#id_user_info_delete').val(),
        data: null,
        headers: { '__RequestVerificationToken': anti_forgery_token, '_ip_user': $('#_ip_user').val() },
        success: function (msg) {
            CloseLoading();
            if (msg.result) {
                SetMessage('1', 'Success delete user');
                window.location.href = '/User/Get';
            }
            else {
                toastr.error('error');
